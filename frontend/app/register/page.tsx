...
export default function RegisterPage() {
  ...
  return <AuthShell eyebrow="New owner account" title="Start with a protected identity." description="Create your WillChain owner account. Email verification is required before sign in, and privileged roles are assigned only through controlled workflows.">
    ...
    <form className="register-form" onSubmit={submit}>
      <div className="register-row"><Field label="First name" required autoComplete="given-name" value={form.first_name} onChange={(event) => update("first_name", event.target.value)} /><Field label="Last name" required autoComplete="family-name" value={form.last_name} onChange={(event) => update("last_name", event.target.value)} /></div>
      <div className="register-row"><Field label="Email address" required type="email" autoComplete="email" placeholder="name@example.com" value={form.email} onChange={(event) => update("email", event.target.value)} /><Field label="Phone number" type="tel" autoComplete="tel" placeholder="Optional" value={form.phone_number} onChange={(event) => update("phone_number", event.target.value)} /></div>
      <div className="register-row"><Field label="Password" required type="password" autoComplete="new-password" value={form.password} onChange={(event) => update("password", event.target.value)} /><Field label="Confirm password" required type="password" autoComplete="new-password" value={form.password_confirmation} onChange={(event) => update("password_confirmation", event.target.value)} /></div>
